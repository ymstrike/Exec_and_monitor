...
#Purpose:Stop process of $USERNAME by givven PID as parmeter from user
...
for proc in $(ps -a -u $USERNAME |awk '{print $1 }') # first word PID 
do
    if [ $proc = $proc_id ];then    # if proc_id found stop the process
     ...
     echo "PID process $proc_id not exist for $USERNAME " |tee  /var/log/$logdir/$logdir.log 
    exit 
     fi
done
